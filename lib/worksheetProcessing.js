...
export const updateRfiSpreadsheet = async (
  ...
  const startRowSpecificIssuesRfi = 22; // Start from row 18 for specific issues
...
export const copyWorksheetToClientWorkbook = async (
  ...
  const templateWorkbookId = await getFileIdByName("RFI Client Template.xlsx");
  ...
  const existingData = await client
    .api(
      `/users/${userId}/drive/items/${sourceWorkbookId}/workbook/worksheets/${sourceWorksheetName}/usedRange`
    )
    .get();
  ...
  try {
    await client
      .api(
        `/users/${userId}/drive/items/${newWorkbookId}/workbook/worksheets('RFI Spreadsheet Template')/range(address='${newRangeAddress}')`
      )
      .patch({
    ...
    try {
      await client
        .api(
          `/users/${userId}/drive/items/${newWorkbookId}/workbook/worksheets('Sheet1')/range(address='${newRangeAddress}')`
        )
        .patch({
          values: cellValuesData,
        });
