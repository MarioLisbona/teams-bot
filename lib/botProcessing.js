...
export const processTestingWorksheet = async (
  ...
  try {
    ...
    // Remove .xlsx extension if it exists
    const cleanClientName = selectedFileData.name.replace(/\.xlsx$/, "");
    ...
    // Find the directory with name matching the client name (without .xlsx)
    const targetDirectory = directories.find(
      (dir) => dir.name === cleanClientName
    );
    const targetDirectoryId = targetDirectory?.id;

    if (!targetDirectoryId) {
      console.log(`No directory found for client: ${cleanClientName}`);
    } else {
      console.log(
        `Found directory for client ${cleanClientName}: ${targetDirectoryId}`
      );
    }
