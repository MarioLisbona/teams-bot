...
export async function getDirectories(directoryName) {
  try {
    ...
    // First, get the ID of the specified directory
    ...
    // Then get the contents of that folder
    const folderContents = await axios.get(
      `https://graph.microsoft.com/v1.0/drives/${process.env.ONEDRIVE_ID}/items/${targetFolder.id}/children`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
