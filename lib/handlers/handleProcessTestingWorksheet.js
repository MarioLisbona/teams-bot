...
import { processTestingWorksheetSelected } from "../utils/auditProcessing.js";
...
export async function handleProcessTestingWorksheet(context, adapter) {
  ...
  if (context.activity.value?.action === "processTestingWorksheetSelected") {
    ...
    // Returns the action "clientWorkbookSelected" when the user selects a file
    await handleDirectorySelection(context, selectedDirectoryId, {
      filterPattern: "Testing",
      action: "clientWorkbookSelected",
    });
  }
  // Handle the actual workbook processing once a workbook has been selected
  else if (context.activity.value?.action === "clientWorkbookSelected") {
    ...
    const newWorkbookName = await processTestingWorksheetSelected(
      context,
      adapter,
      combinedFileData
    );
