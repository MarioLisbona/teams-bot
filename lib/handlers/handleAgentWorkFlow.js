...
/**
 * Handles the human validation needed after the Agent has completed the workflow
 ...
 */
export async function handleHumanValidation(
  adapter,
  serviceUrl,
  conversationId,
  channelId,
  ...
  console.log("handleHumanValidation function called");

  const validationCard = createHumanValidationCard(validations, jobId);
...
export async function handleValidateWorkflowStep(context) {
  ...
  await context.sendActivity({
    attachments: [
      {
        contentType: "application/vnd.microsoft.card.adaptive",
        content: createHumanValidationCard(updatedValidations, jobId),
      },
    ],
  });
}
