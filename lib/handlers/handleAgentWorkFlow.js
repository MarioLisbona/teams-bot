 ...
 * @param {Object} context.activity.value.validationsRequired - Current validation requirements
...
export async function handleHumanValidationSteps(context) {
  try {
    const { validationType, validationsRequired, completedValidations, jobId } =
    ...
    try {
      ...
      try {
        ...
        const allValidationsComplete = Object.keys(validationsRequired).every(
...
export async function handleAuditCommandSubmitted(context) {
  try {
    const auditCommand = context.activity.value.auditCommand;
    await createTeamsUpdate(
      context,
    ...
    // TODO: Create a post request to the agent with auditCommand
  } catch (error) {
    console.error("Failed to process audit command:", error);
    throw error;
  }
}
