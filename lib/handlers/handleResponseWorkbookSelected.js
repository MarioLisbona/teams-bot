...
export const handleResponseWorkbookSelected = async (context) => {
  try {
    ...
    console.log("client response length:", processedClientResponses.length);

    // Process client responses in batches of 6
    const batchSize = 6;
    const allResponses = [];
    ...
    // Use the combined results for Excel update
    const updatedResponseData = allResponses;
