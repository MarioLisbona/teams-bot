  ...
  if (context.activity.type === "message") {
    ...
    switch (action || userMessage) {
      ...
      case "validate":
        ...
        // First, immediately send a new validation card with the button disabled/removed
        await context.sendActivity({
          attachments: [
            {
              contentType: "application/vnd.microsoft.card.adaptive",
              content: createWorkflow1ValidationCard({
                ...currentValidations,
                [`${documentType}Processing`]: true, // Add a processing state
              }),
            },
          ],
        });

        // Then send the "validating" message
        await context.sendActivity({
          type: "message",
          text: `🔄 Validating ${documentType}...`,
        });
        ...
        await context.sendActivity({
          type: "message",
          text: `✅ ${documentType} validation complete!`,
        });
