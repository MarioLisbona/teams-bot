...
export async function handleMessages(adapter, context) {
  ...
  try {
    switch (action || userMessage) {
      ...
      // Returns the action "processTestingWorksheetSelected" when the user selects the "Process Testing Worksheet" button
      // Returns the action "processClientResponsesSelected" when the user selects the "Process Client Responses" button
      case "clientSelected":
        await handleSelectClientDirectory(context);
        break;

      case "processTestingWorksheetSelected":
      case "selectClientWorkbook":
        await handleProcessTestingWorksheet(context, adapter);
        break;

      case "processClientResponsesSelected":
