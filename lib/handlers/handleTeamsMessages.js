...
import { handleApproveRejectImages } from "./handleApproveRejectImages.js";
import { handleValidateWorkflowStep } from "./handleValidateWorkflowStep.js";
import {
  createHelpCard,
...
export async function handleMessages(context) {
  ...
  if (context.activity.type === "message") {
    ...
    switch (action || userMessage) {
      ...
      // Displays the validation card for workflow step 1
      case "validate":
        await handleValidateWorkflowStep(context);
        break;

      // Handle image TESTING approval/rejection of images present in the validation card
      case "approve":
      case "reject":
        await handleApproveRejectImages(context);
        break;
