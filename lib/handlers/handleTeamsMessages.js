    ...
    if (context.activity.type === "message") {
      ...
      try {
        switch (action || userMessage) {
          ...
          case "auditCommandSubmitted":
            await handleAuditCommandSubmitted(context);
...
export async function handleTextMessages(context, userMessage) {
  try {
    if (userMessage.toLowerCase() === "help") {
      ...
      console.log(`Echoing message inside handleTextMessages: ${userMessage}`);
      try {
        await createTeamsUpdate(context, userMessage, "", "🤖");
      } catch (error) {
 ...
 * "Rfi" command initiates the RFI processing for both the Testing workbook and Response Workbooks
 * "RfiClientSelected" command is triggered when a client is selected from the RFI client selection card
 *
 * @description
  ...
  try {
    ...
    switch (command) {
      case "rfi":
        ...
        break;
      case "audit":
        commandAction = "Begin Workflow Audit";
        break;
      default:
    ...
    if (command === "audit") {
      try {
        // Send adaptive card to collect audit command
        await context.sendActivity({
          attachments: [
            {
              contentType: "application/vnd.microsoft.card.adaptive",
              content: createAuditCommandCard(),
            },
          ],
        });
      } catch (error) {
        console.error("Failed to show audit command card:", error);
        throw error;
      }
    } else {
      try {
        ...
        throw error; // Remove error wrapping to prevent multiple messages
      }
    }
  } catch (error) {
