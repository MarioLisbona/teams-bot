...
/**
 * Retrieves job directories from client's Evidence Packs folder.
 *
 * @description
 * Hierarchical directory retrieval:
 * 1. Authenticates with Microsoft Graph
 * 2. Accesses specified client subdirectory
 * 3. Locates "Evidence Packs" folder
 * 4. Retrieves all job directories
 * 5. Filters and extracts folder information
 *
 * @param {string} directoryName - Client directory name (for error context)
 * @param {string} subDirectoryID - SharePoint ID of client directory
 *
 * @throws {Error} When authentication fails
 * @throws {Error} When client directory access fails
 * @throws {Error} When Evidence Packs folder not found
 * @throws {Error} When job directories cannot be retrieved
 * @returns {Promise<Array<Object>>} Array of job directory objects:
 *   - id: SharePoint folder identifier
 *   - name: Job folder name
 *
 * @example
 * const jobs = await getClientJobDirectories("Client A", "dir123");
 * // Returns: [{ name: "Job 2024-001", id: "job123" }, ...]
 *
 * @requires axios
 * @requires SHAREPOINT_SITE_ID environment variable
 */
