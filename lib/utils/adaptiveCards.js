 ...
 */
export const createWorkflow1ValidationCard = (validations) => {
  const createValidationText = (item, exists) => {
    return `${item}: ${exists ? "✅" : "❌"}`;
  };
    ...
    body: [
      {
        ...
        items: [
          {
            type: "TextBlock",
            text: "Document Validation Checklist",
            size: "large",
            weight: "bolder",
            color: "accent",
          },
          {
            ...
            textFormat: "markdown",
            text: [
              createValidationText("Nom Form", validations.nomForm),
              createValidationText(
                "Site Assessment Report",
                validations.siteAssessment
              ),
              createValidationText("Tax Invoice", validations.taxInvoice),
              createValidationText("CCEW", validations.ccew),
              createValidationText(
                "Installer Declaration",
                validations.installerDec
              ),
              createValidationText("CoC", validations.coc),
              createValidationText("GTP", validations.gtp),
            ].join("\n\n"),
            wrap: true,
            spacing: "medium",
          },
