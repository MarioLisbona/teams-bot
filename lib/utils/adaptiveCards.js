...
export const createWorkflow1ValidationCard = (validations) => {
  const createValidationItem = (key, label, exists) => {
    return {
      type: "ColumnSet",
      columns: [
        {
                  ...
                  actions: [
                    {
                      ...
                      data: {
                        action: "validate",
                        documentType: key,
                        currentValidations: validations, // Include current state
                      },
                    },
                  ],
                },
    ...
    body: [
      {
        ...
        items: [
          ...
          ...Object.entries({
            ...
            installerDec: ["Installer Declaration", validations.installerDec],
            coc: ["CoC", validations.coc],
            gtp: ["GTP", validations.gtp],
          }).map(([key, [label, exists]]) =>
            createValidationItem(key, label, exists)
          ),
        ],
        padding: "medium",
      },
