...
import { azureGptQuery } from "./azureGpt.js";
            ...
            .map((row, rowIndex) => {
              try {
                ...
                if (!rfiData.issuesIdentified || !rfiData.acpResponse) {
                  console.warn(
                    `No issues identified or ACP response found - Skipping row ${
                      rowIndex + start + 4
                    }` // +4 to account for header rows and 0-based index
                  );
                  return null;
                }
...
export const getSharepointUrl = async (client, workbookId) => {
  const fileInfo = await client
    .api(`/sites/${process.env.SHAREPOINT_SITE_ID}/drive/items/${workbookId}`)
    .get();
