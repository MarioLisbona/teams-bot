      ...
      try {
        ...
        if (updatedRfiCellData.length > 0) {
          try {
            ...
            // Get the SharePoint URL for the new workbook
            const fileInfo = await client
              .api(
                `/sites/${process.env.SHAREPOINT_SITE_ID}/drive/items/${newWorkbookId}`
              )
              .get();

            const fileUrl = fileInfo.webUrl;
            console.log("File URL before Teams update:", fileUrl);
